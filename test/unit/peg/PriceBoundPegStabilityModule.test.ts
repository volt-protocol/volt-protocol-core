...
describe('PriceBoundPegStabilityModule', function () {
  ...
  describe('Mint', function () {
    describe('Sells Token for Volt', function () {
      ...
      it('exchanges 1000 DAI for 975 volt as fee is 250 bips and exchange rate is 1:1', async () => {
      ...
      it('exchanges 1000 DAI for 975 volt as fee is 250 bips and exchange rate is 1:1 when to address is not msg.sender', async () => {
        const oneK = toBN(1000);
        const newMintFee = 250;
        await psm.connect(impersonatedSigners[governorAddress]).setMintFee(newMintFee);

        const userStartingvoltBalance = await volt.balanceOf(governorAddress);
        const psmStartingAssetBalance = await asset.balanceOf(psm.address);
        const expectedMintAmountOut = 975;
        ...
        const endingUserAssetBalance = await asset.balanceOf(userAddress);
        const userEndingvoltBalance = await volt.balanceOf(governorAddress);
        const psmEndingAssetBalance = await asset.balanceOf(psm.address);
      ...
      it('exchanges 1000 DAI for 975 volt as fee is 300 bips and exchange rate is 1:1', async () => {
        const oneK = toBN(1000);
      ...
      it('exchanges 1000 DAI for 975 volt as mint fee is 250 bips and exchange rate is 1:1', async () => {
        const oneK = toBN(1000);
        const newMintFee = 250;
        await psm.connect(impersonatedSigners[governorAddress]).setMintFee(newMintFee);
      ...
      it('exchanges 1000 DAI for 950 volt as mint fee is 50 bips and exchange rate is 1DAI:1.2volt', async () => {
        const oneK = toBN(1000);
        const newMintFee = 50;
        await psm.connect(impersonatedSigners[governorAddress]).setMintFee(newMintFee);
      ...
      it('should not mint when expected amount out is 3x greater than minting buffer cap and all psm volt is used', async () => {
      ...
      it('should not mint when expected amount out is 1 more than minting buffer cap and all psm volt is used', async () => {
  ...
  describe('Redeem', function () {
    describe('Sells volt for Token', function () {
      beforeEach(async () => {
      ...
      it('exchanges 1000 volt for 975 DAI as fee is 250 bips and exchange rate is 1:1', async () => {
        const oneK = toBN(1000);
        ...
        await volt.connect(impersonatedSigners[minterAddress]).mint(userAddress, oneK);
        await volt.connect(impersonatedSigners[userAddress]).approve(psm.address, oneK);
      ...
      it('exchanges 1000 volt for 975 DAI as fee is 250 bips and exchange rate is 1:1 and specifies a to address not msg.sender', async () => {
        const oneK = toBN(1000);
        const newRedeemFee = 250;
        await psm.connect(impersonatedSigners[governorAddress]).setRedeemFee(newRedeemFee);
      ...
      it('redeem succeeds when user has enough funds', async () => {
        ...
        const startingPSMvoltBalance = await volt.balanceOf(psm.address);
        ...
        const userEndingAssetBalance = await asset.balanceOf(userAddress);
        const endingUservoltBalance = await volt.balanceOf(userAddress);

        expect(userEndingAssetBalance.sub(userStartingAssetBalance)).to.be.equal(expectedAssetAmount);
        expect(endingUservoltBalance).to.be.equal(startingUservoltBalance.sub(mintAmount));
      ...
      it('redeem succeeds when user has enough funds and DAI is $1.019 with .1 volt', async () => {
        const pointOnevolt = ethers.constants.WeiPerEther.div(10);
        await oracle.setExchangeRateScaledBase(ethers.constants.WeiPerEther.mul(1019).div(1000));
        await volt.connect(impersonatedSigners[minterAddress]).mint(userAddress, pointOnevolt);
        await volt.connect(impersonatedSigners[userAddress]).approve(psm.address, pointOnevolt);
        ...
        await psm.connect(impersonatedSigners[userAddress]).redeem(userAddress, pointOnevolt, expectedAssetAmount);

        const endingUservoltBalance = await volt.balanceOf(userAddress);
        const endingUserAssetBalance = await asset.balanceOf(userAddress);

        expect(endingUservoltBalance).to.be.equal(startingUservoltBalance.sub(pointOnevolt));
        expect(endingUserAssetBalance).to.be.equal(startingUserAssetBalance.add(actualAssetAmount));
        expect(await volt.balanceOf(psm.address)).to.be.equal(pointOnevolt.add(startingPSMvoltBalance));
        expect(await psm.buffer()).to.be.equal(bufferCap);
      });

      it('redeem succeeds when user has enough funds and DAI is $1.019 with .01 volt', async () => {
        const pointOnevolt = ethers.constants.WeiPerEther.div(100);
        await oracle.setExchangeRateScaledBase(ethers.constants.WeiPerEther.mul(1019).div(1000));
        await volt.connect(impersonatedSigners[minterAddress]).mint(userAddress, pointOnevolt);
        await volt.connect(impersonatedSigners[userAddress]).approve(psm.address, pointOnevolt);
      ...
      it('fails when token is not approved to be spent by the PSM', async () => {
        await volt.connect(impersonatedSigners[minterAddress]).mint(userAddress, 100);
        await expectRevert(
