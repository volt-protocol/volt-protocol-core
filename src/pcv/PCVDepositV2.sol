...
abstract contract PCVDepositV2 is IPCVDepositV2, CoreRefV2 {
    ...
    function deposit() public whenNotPaused globalLock(2) {
        ...
        /// retrieve the non-decimal normalized balance
        /// if PCV Oracle is not set, revert
        uint256 lastRecordedBalance = pcvOracle().lastRecordedPCVRaw(
            address(this)
        );
    ...
    function accrue() external whenNotPaused globalLock(2) returns (uint256) {
        ...
        return balance(); /// return current pcv amount
    }
    ...
    function withdrawAll(address to) external onlyPCVController globalLock(2) {
        ...
        int256 recordedBalance = lastRecordedBalance.toInt256(); /// cast to int256
